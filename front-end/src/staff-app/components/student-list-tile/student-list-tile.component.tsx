...
interface Props {
  isRollMode?: boolean
  student: Person
  onStateChange: (value: RolllStateType, id: number) => void
}
export const StudentListTile: React.FC<Props> = ({ isRollMode, student, onStateChange }) => {
  return (
    <S.Container>
      ...
      {isRollMode && (
        <S.Roll>
          <RollStateSwitcher initialState={student.current_roll_state} onStateChange={(value) => onStateChange(value, student.id)} />
        </S.Roll>
      )}
    </S.Container>
  )
